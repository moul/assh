...
class TestParseOptions(unittest.TestCase):
    ...
    def test_with_args_without_options(self):
        self.assertRaises(
            ValueError, _advanced_ssh_config_parse_options, 'localhost'
        )

    def test_with_args_with_options(self):
        self.assertRaises(
            ValueError, _advanced_ssh_config_parse_options, '--port=22',
            'localhost'
        )
        self.assertRaises(
            ValueError, _advanced_ssh_config_parse_options, 'localhost',
    ...
    def test_unkown_options(self):
        with RedirectStdStreams(stdout='devnull', stderr='devnull'):
            self.assertRaises(
                SystemExit, _advanced_ssh_config_parse_options, '--toto=titi'
            )
    ...
    def test_option_valid_host(self):
        for host in ('1.2.3.4',
                     ...
                     '::1',
                     'fe80::1%lo0'):
            options = _advanced_ssh_config_parse_options(
                '--host={}'.format(host)
            )
    ...
    def test_option_invalid_host(self):
        with RedirectStdStreams(stdout='devnull', stderr='devnull'):
            self.assertRaises(
                SystemExit, _advanced_ssh_config_parse_options, '-H'
            )
            self.assertRaises(
                SystemExit, _advanced_ssh_config_parse_options, '--host'
            )

        for host in ('',):
            self.assertRaises(
                ValueError, _advanced_ssh_config_parse_options,
                '--host={}'.format(host)
            )
            self.assertRaises(
                ValueError, _advanced_ssh_config_parse_options, '-H', host
            )

    def test_option_valid_port(self):
        for port in (22, 1, 65535):
            options = _advanced_ssh_config_parse_options(
                '--host=localhost', '--port={}'.format(port)
            )
    ...
    def test_option_invalid_port(self):
        with RedirectStdStreams(stdout='devnull', stderr='devnull'):
            self.assertRaises(
                SystemExit, _advanced_ssh_config_parse_options,
                '--host=localhost', '--port'
            )
            self.assertRaises(
                SystemExit, _advanced_ssh_config_parse_options,
